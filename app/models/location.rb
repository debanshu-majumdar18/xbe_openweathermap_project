class Location < ApplicationRecord
  ...
  def average_air_quality_index(period_in_days = 30)
    PollutionConcentration.where(location: self).order(created_at: :desc).limit(period_in_days).average(:aqi)
  end

  def self.average_aqi_by_state
    Location.joins(:pollution_concentrations).group(:state).average(:aqi)
  end
